...
from .exporter import _write_header_row, _set_column_widths, _body_font, _row_fill, COLOUR_ALT_ROW
    ...
    for idx, game in enumerate(matched_games, start=1):
        ...
        try:
            ...
            game.update({
                ...
                "completion_pct": round((prog["earned"] / prog["total"]) * 100, 1) if prog["total"] > 0 else 0.0,
...
def _export_franchise_excel(keyword: str, df: pd.DataFrame):
    ...
    ws_stats.cell(row=1, column=1, value=f"Franchise Report: {keyword.upper()}").font = Font(bold=True, size=14)
    ...
    def write_data_sheet(title, data_df):
        ...
        for row_idx, (_, row) in enumerate(data_df.iterrows(), start=2):
            ...
            for col_idx, (_, key, _) in enumerate(cols, start=1):
                val = row.get(key, "")
                if key == "is_mastered": val = "Yes" if val else "No"
                cell = ws.cell(row=row_idx, column=col_idx, value=val)
                cell.font = _body_font()
                if alt_colour: cell.fill = _row_fill(alt_colour)
